...
void GraphManager::backward(Variable &start) {
    ...
    while (!fifo_.empty()) {
        ...
        const auto &iter = this->_variableCreateBy.find(fifo_.front());
        fifo_.pop();
        if (iter == this->_variableCreateBy.end()) {
            std::cout << "是叶子节点,continue" << std::endl;
            continue;
        }
        std::cout << "被Operator=" << iter->second << "调用" << std::endl;
        std::shared_ptr<OperatorBase> &invoke_op_ = this->_operators.at(iter->second);
        invoke_op_->backward();
